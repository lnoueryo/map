...
class Geohash():
	...
	def bbox(self, hashcode):
		...
		if hasattr(float, "fromhex"):
			latitude_delta  = 180.0/(1<<lat_length)
			longitude_delta = 360.0/(1<<lon_length)
			latitude = self._int_to_float_hex(lat, lat_length) * 90.0
			longitude = self._int_to_float_hex(lon, lon_length) * 180.0
	...
	def neighbors(self, hashcode):
		...
		for tlon in (lon-1, lon+1):
			code = self._encode_i2c(tlat,tlon,lat_length,lon_length)
			if code:
				ret.append(code)
	...
	def expand(self, hashcode):
		ret = self.neighbors(hashcode)
		ret.append(hashcode)
		return ret

	def _uint64_interleave(self, lat32, lon32):
		intr = 0
	...
	def _uint64_deinterleave(self, ui64):
		lat = lon = 0
	...
	def encode_uint64(self, latitude, longitude):
		if latitude >= 90.0 or latitude < -90.0:
		...
		return self._uint64_interleave(lat, lon)

	def decode_uint64(self, ui64):
		lat,lon = self._uint64_deinterleave(ui64)
		return (180.0*lat/(1<<32) - 90.0, 360.0*lon/(1<<32) - 180.0)

	def expand_uint64(self, ui64, precision=50):
		...
		if lat & lat_grid:
			if lon & lon_grid:
				ui64 = self._uint64_interleave(lat-lat_grid, lon-lon_grid)
				...
				if precision%2==0:
					...
					if lat + lat_grid < 0xFFFFFFFF:
						...
						ui64 = self._uint64_interleave(lat+lat_grid, lon)
						ranges.append((ui64, ui64 + (1<<(64-precision))))
						ui64 = self._uint64_interleave(lat+lat_grid, lon+lon_grid)
						ranges.append((ui64, ui64 + (1<<(64-precision))))
				else:
					...
					ui64 = self._uint64_interleave(lat, lon+lon_grid)
					ranges.append((ui64, ui64 + (1<<(64-precision))))
					ui64 = self._uint64_interleave(lat-lat_grid, lon+lon_grid)
					ranges.append((ui64, ui64 + (1<<(64-precision))))
			else:
			...
			if lon & lon_grid:
				...
				ui64 =self._uint64_interleave(lat, lon)
				ranges.append((ui64, ui64 + (1<<(64-precision+2))))
				if precision%2==0:
