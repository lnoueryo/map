...
export default Vue.extend({
  data(): dataType {
    return {
      ...
      horizontalAxis: [
        1920, 1925, 1930, 1935, 1940, 1944, 1945, 1946, 1947, 1948, 1950, 1955,
        1960, 1965, 1970, 1975, 1980, 1985, 1990, 1995, 2000, 2005, 2010, 2015,
        2016, 2017, 2018, 2019,
      ], //lengthをverticalAxisと揃える
  ...
  methods: {
    init() {
      ...
      this.canvas.context = this.canvas.chart.getContext(
        "2d"
      ) as CanvasRenderingContext2D;
      this.canvas.chart.width =
        this.canvas.chart.getBoundingClientRect().width * 10;
      this.canvas.chart.height =
        this.canvas.chart.getBoundingClientRect().height * 10;
    ...
    makeHorizontalAxis(
      ...
      for (let i = this.chartConfig.dividedNum + 1; i > 0; i--) {
        ...
        context.stroke();
        if (i < this.chartConfig.dividedNum && i > 0) {
          this.chartConfig.heights.push(i * dividedHeight);
        }
      }
    },
    makeHorizontalAxisText(canvas: Canvas) {
      ...
      for (
        let i = 0;
        i < this.horizontalAxisFilter.length;
        i = i + (1 * 8) / this.horizontalAxisFilter.length
      ) {
        widths.push(width + width * i);
      }
      for (let i = 0; i < this.horizontalAxisFilter.length; i++) {
        context.fillText(
          this.horizontalAxisFilter[i],
          widths[i],
          9.5 * height,
          2000
        ); //10はcanvasからはみ出してしまう
    ...
    makeVerticalAxis(
      ...
      context.lineTo(
        pixelWidth,
        (this.chartConfig.dividedNum - 1) * dividedHeight
      );
      context.closePath();
      context.stroke();
    },
    makeVerticalAxisText(canvas: Canvas) {
      ...
      let values = [];
      for (
        let i = chart.min - difference;
        i < chart.max + difference + 1;
        i = i + difference
      ) {
    ...
    makeDot(
      ...
      const difference = this.horizontalAxisDiff();
      const startLine =
        ((this.chartConfig.dividedNum - 2) * pixelHeight) /
        this.chartConfig.dividedNum;
      const heightPerNum =
        pixelHeight / this.chartConfig.dividedNum / difference;
      for (let i = 0; i < widths.length; i++) {
        const diffFromStartLine = this.verticalAxisFilter[i] - chart.min;
        context.beginPath();
        context.arc(
          widths[i],
          startLine - heightPerNum * diffFromStartLine,
          40,
          0,
          2 * Math.PI,
          true
        ...
        if (i !== widths.length) {
          ...
          context.lineTo(
            widths[i + 1],
            startLine -
              heightPerNum * (this.verticalAxisFilter[i + 1] - chart.min)
          );
        }
        context.stroke();
        context.restore();
    ...
    chartMaxMin() {
      const max = this.verticalAxisFilter.reduce((a: number, b: number) => {
        return Math.max(a, b);
      });
      const min = this.verticalAxisFilter.reduce((a: number, b: number) => {
        return Math.min(a, b);
      });
    ...
    horizontalAxisDiff() {
      const chart = this.chartMaxMin();
      const difference =
        (chart.max - chart.min) / (this.chartConfig.dividedNum - 4);
      return difference;
    },
    drawTriangle() {
    ...
    resetChart() {
      ...
      this.data = Object.assign({}, this.data, populationData[this.index]);
      this.verticalAxis = [];
      this.verticalAxis = this.verticalAxis.concat(
        populationData[this.index].population
      );
      this.chartConfig = Object.assign({}, this.chartConfig, { heights: [] });
    },
  },
});
</script>
