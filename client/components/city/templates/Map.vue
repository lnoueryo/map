...
export default Vue.extend({
            ...
            handler(v) {
                if(!v) {
                    if (this.selectedCompanyItems.length !== 0) {
                        this.$store.dispatch('city/selectedCompanyItems', []);
                    }
                    if (this.selectedLineItems.length !== 0) {
                        this.$store.dispatch('city/selectedLineItems', []);
                    }
                }
            }
        },
    ...
    async mounted() {
        ...
        this.$mapConfig.map.addListener("bounds_changed", () => {
            ...
            timer = setTimeout(() => {
                this.showMarkers();
                this.$store.dispatch('city/getCity', {mapCenter: mapCenter, zoom: zoom});
                this.$store.dispatch('city/getCurrentBounds', bounds);
            }, 1000);
    ...
    methods:{
        ...
        stationMarkerFunction(marker: google.maps.Marker, station: {name: string}) {
            marker.addListener("click", async () => {
                this.$store.dispatch('city/selectMarker',station);
        ...
        cityMarkerFunction(marker: google.maps.Marker, city: {name: string}) {
                marker.addListener("click", async (e: google.maps.MapMouseEvent) => {
                    await this.$store.dispatch('city/searchCityCode', e);
                    // this.$store.dispatch('city/selectMarker', word);
        ...
        spotMarkerFunction(marker: google.maps.Marker, spot: {name: string}) {
            marker.addListener("click", async () => {
                this.$store.dispatch('city/selectMarker', spot);
