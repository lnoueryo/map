<template>
  <div>
    <div v-for="(layout, i) in analysisData.layouts" :key="i">
      <div class="d-flex align-center text-center py-4" v-if="i == 0 && !tablet">
        <div class="town-name">間取り</div>
        <div class="columns">
      ...
      <div class="table-row text-center py-2">
        <div class="town-name">
          <div>{{layout.type}}</div>
          <div>{{roundPercentage(layout.count_ratio)}}%</div>
        </div>
        <div class="columns">
          ...
          <div class="table-data py-1" v-for="(column, j) in layout.columns" :key="j">
