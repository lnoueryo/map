...
export default Vue.extend({
    ...
    methods:{
        init(){
            ...
            this.canvas.chart.width = this.canvas.chart.getBoundingClientRect().width*10
            this.canvas.chart.height = this.canvas.chart.getBoundingClientRect().height*10
        ...
        makeChart(){
            ...
            context.save();
            const config: Context = {context: context as CanvasRenderingContext2D, lineWidth: 0.015*width, fontSize: 0.216*width, fillStyle: 'blue', textAlign: 'right', textBaseline: 'top'}
            this.setContext(config)

            context.fillText(this.data.city as string, width, 0.05*dividedHeight, 2000);
        ...
        makeHorizontalAxisText(canvas: Canvas){
            ...
            const config: Context = {context: context as CanvasRenderingContext2D, lineWidth: 0.015*width, fontSize: 0.216*width, fillStyle: 'blue', textAlign: 'center', textBaseline: 'top'}
            this.setContext(config)
            let widths = []
        ...
        makeVerticalAxisText(canvas: Canvas){
            const context = canvas.context as CanvasRenderingContext2D;
            const width = canvas.width as number
            const config: Context = {context: context as CanvasRenderingContext2D, lineWidth: 0.015*width, fontSize: 0.216*width, fillStyle: 'blue', textAlign: 'right', textBaseline: 'middle'}
            this.setContext(config)
        ...
        makeDot(context: CanvasRenderingContext2D, width: number, pixelHeight: number){
            ...
            const config: Context = {context: context as CanvasRenderingContext2D, lineWidth: 0.03*width, fontSize: 0.216*width, fillStyle: '#cacaca', textAlign: 'right', textBaseline: 'middle'}
            this.setContext(config)
            context.strokeStyle = '#cacaca';
            const chart = this.chartMaxMin();
            const difference = this.horizontalAxisDiff();
        ...
        resetChart(){
            ...
            this.verticalAxis = [];
            this.verticalAxis = this.verticalAxis.concat(populationData[this.index].population)
